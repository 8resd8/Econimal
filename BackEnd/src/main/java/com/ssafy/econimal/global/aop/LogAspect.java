...
public class LogAspect {
	...
	@Before("execution(* com.ssafy.econimal..controller..*(..))")
	public void logBefore(JoinPoint joinPoint) {
		...
		logMessage.append("\n").append(REQUEST_SEPARATOR).append(CONTROLLER_LOG_START).append(REQUEST_SEPARATOR).append("\n")
			.append("▶ [Request ID] : ").append(requestId).append("\n")
			.append("▶ [Request URI]: ").append(request.getRequestURI()).append("\n")
			.append("▶ [Method]     : ").append(methodName).append("\n")
			.append("▶ [Params]     : ").append(args.length > 0 ? Arrays.toString(args) : "요청값 없음").append("\n")
			.append(LINE_SEPARATOR);

		log.info(logMessage.toString());
	}

	@AfterReturning(pointcut = "execution(* com.ssafy.econimal..controller..*(..))", returning = "result")
