...
const EventAlert = ({ isActive, className = '' }: EventAlertProps) => {
  ...
  return (
    <div
      ...
      className={`absolute w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-2xl sm:text-base md:text-xl lg:text-2xl font-bold animate-bounce ${className}`} // 바운스
    >
      !
    </div>
  );
