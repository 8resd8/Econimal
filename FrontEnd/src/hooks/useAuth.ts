...
export const useAuth = () => {
  ...
  const setupRefreshTimer = (expiresIn: number) => {
    ...
    console.log(
      `토큰 만료 시간: ${expiresIn}ms, ${refreshDelay}ms 후 갱신 예정 (만료 ${Math.min(
        10000,
        expiresIn,
      )}ms 전)`,
    );
  ...
  const refreshToken = async () => {
    try {
      ...
      console.log(
        '새 토큰 설정 완료, 만료 시간:',
        new Date(Date.now() + res.data.timeToLive).toISOString(),
      );
      ...
      window.dispatchEvent(
        new CustomEvent('token-refreshed', {
          detail: {
            ...
            timeToLive: res.data.timeToLive,
          },
        }),
      );
  ...
  const handleLogout = (redirectToLogin = true) => {
    ...
    // 캐릭터 디테일 정보 삭제
    localStorage.removeItem('char-status');
    localStorage.removeItem('myCharInformation');
  ...
  const validateEmail = async (email: string) => {
    try {
      const response = await axiosInstance.post('/users/email-validation', {
        email,
      });
  ...
  const verifyEmailCode = async (email: string, authCode: string) => {
    try {
      await axiosInstance.post('/users/email/password/reset/confirm', {
        email,
        authCode,
      });
