...
start_time = time.time()
json_df = final_df.select(F.to_json("result").alias("jsonString"))
json_df = json_df.withColumn("id", F.monotonically_increasing_id())
print("[Redis] Redis 저장용 데이터 준비 시간: {:.2f}초".format(time.time() - start_time))

print("[Redis] Redis 저장 시작")
start_time = time.time()
json_df.write \
    .format("org.apache.spark.sql.redis") \
    .option("table", "climate_data") \
